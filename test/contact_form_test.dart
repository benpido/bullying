...
void main() {
  ...
  testWidgets('saving with one valid phone stores contact', (tester) async {
    final service = MockContactService();
    when(() => service.getContacts()).thenAnswer((_) async => []);
    ...
    final contactsArg = verify(() => service.setContacts(captureAny()))
        .captured
        .single as List<ContactModel>;
    expect(contactsArg.length, 1);
